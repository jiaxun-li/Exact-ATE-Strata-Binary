...
def confidence_interval_permute(matrix_n,alpha=0.05,replication=100):
    ...
    for matrix_N in product(*matrix_N_list):
        if sum(matrix_N[i][2]+matrix_N[i][0] for i in range(k))!=0:
            tau=sum(matrix_N[i][1]+matrix_N[i][0] for i in range(k))/sum(matrix_N[i][2]+matrix_N[i][0] for i in range(k))
        else:
            tau=100000
        j+=1
        temp=j/s
...
#     [8,14,2,24]
# ]
# print(confidence_interval_permute(matrix_n,replication=1000))