...
# for treatment effects on a binary outcome
def confidence_interval_hypergeo_ws(matrix_n: list, alpha: float=0.05) -> list:
    '''
    calculate the confidence bound for the ATE with given observed table matrix_n using Wendell and Scheme 
    ...
    sams_control=[i[2]+i[3] for i in matrix_n]
    found_treat=[i[0] for i in matrix_n]
    found_control=[i[2] for i in matrix_n]
    ub_treat,temp=strat_ci_bisect(strata,sams_treat,found_treat,alternative='upper',cl=1-alpha/4, p_value=strat_test_ws)
    lb_treat,temp=strat_ci_bisect(strata,sams_treat,found_treat,alternative='lower',cl=1-alpha/4, p_value=strat_test_ws)
    ub_control,temp=strat_ci_bisect(strata,sams_control,found_control,alternative='upper',cl=1-alpha/4, p_value=strat_test_ws)
    lb_control,temp=strat_ci_bisect(strata,sams_control,found_control,alternative='lower',cl=1-alpha/4, p_value=strat_test_ws)
    N=np.sum(matrix_n)
    return [(lb_treat/ub_control),(ub_treat/lb_control)]

def confidence_interval_hypergeo_fast(matrix_n: list, alpha: float=0.05) -> list:
    '''
    ...
    found_treat=[i[0] for i in matrix_n]
    found_control=[i[2] for i in matrix_n]
    ub_treat,temp=strat_ci(strata,sams_treat,found_treat,alternative='upper',cl=1-alpha/4)
    lb_treat,temp=strat_ci(strata,sams_treat,found_treat,alternative='lower',cl=1-alpha/4)
    ...
    N=np.sum(matrix_n)
    return [(lb_treat/ub_control)*(N-ub_control)/(N-lb_treat),(ub_treat/lb_control)*(N-lb_control)/(N-ub_treat)]
