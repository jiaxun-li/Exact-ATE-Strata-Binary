...
def confidence_interval_permute(matrix_n,alpha=0.05,replication=100):
    ...
    N=[np.sum(matrix_n[i]) for i in range(k)]
    n=[matrix_n[i][0]+matrix_n[i][1] for i in range(k)]
    tau_hat=2*sum(matrix_n[i][0]-matrix_n[i][2] for i in range(k))
    U=-sum(N)
    L=sum(N)
    tau_temp=[]
    ...
    total=1
    for i in range(k):
        matrix_N_temp=Ni_generator(matrix_n[i])

        tau_temp.append(range(min(matrix_N_temp.keys()),max(matrix_N_temp.keys())+1))
        total*=(max(matrix_N_temp.keys())-min(matrix_N_temp.keys())+1)
        matrix_N_core_temp.append(Ni_core_generator(matrix_N_temp,N[i]))
    j=0
    for tau_vector in product(*tau_temp):
        j+=1
        tau=sum(tau_vector)
        temp=[]
        for i in range(k):
            temp.append(matrix_N_core_temp[i][tau_vector[i]])
        for matrix_N in product(*temp):
